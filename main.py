...
async def meat(ctx, value: str):
  ...
  try:
    ...
    if str(ve) == "True":
      ...
      await ctx.reply("minimal mikir kontol, minimal 5 meat",
                      mention_author=False)


# @client.tree.command(name="ppkm", description="ngitung honor berdasarkan ppkm")
@client.hybrid_command(name="ppkm",
                       description="ngitung honor berdasarkan ppkm")
async def ppkm(
  ctx,
  target_honor: str,
  ...
  if target_honor * 1000 < convert_to_int("1b"):
    await ctx.reply(
      "Target honor minimal 1B",
      mention_author=False,
    )
    await ctx.send(
      "https://media.discordapp.net/attachments/627855503921512487/927122622746087434/unknown.png"
    )
    return
  HONOR_PRELIM = 30000
...
async def help(ctx, command: typing.Optional[str] = None):
  ...
  commands = {
    "meat":
    "Menghitung jumlah honor NM90, NM95, NM100, NM150 dan NM200 dari jumlah meat yang diinputkan.",
    "meathonor":
  ...
  if command is None:
    help_embed = discord.Embed(
      title="Jem-BOT Commands",
      description=
      "Gunakan`!help <command>` untuk mendapatkan info lebih lanjut dari daftar command dibawah."
    )
    for commandItem in commands:
      help_embed.add_field(name=commandItem,
                           value=commands[commandItem],
                           inline=False)
    ...
    help_embed = discord.Embed(title="Jem-BOT Commands")
    help_embed.add_field(name="Error",
